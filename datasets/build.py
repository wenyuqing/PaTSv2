...
def train_pipeline_ssv2(config):
    scale_resize = int(256 / 224 * config.DATA.INPUT_SIZE)
    train_pipeline = [
        dict(type='DecordInit'),
        dict(type='SampleFrames', clip_len=config.DATA.CLIP_LEN, frame_interval=config.DATA.SAMPLE_RATE,
             num_clips=config.DATA.NUM_FRAMES, frame_uniform=config.DATA.FRAME_UNIFORM),
...
def train_pipeline_ssv2_swin(config):
    scale_resize = int(256 / 224 * config.DATA.INPUT_SIZE)
    train_pipeline = [
        dict(type='DecordInit'),
        dict(type='SampleFrames', clip_len=config.DATA.CLIP_LEN, frame_interval=config.DATA.SAMPLE_RATE,
...
def train_pipeline_ssv2_slowfast(config):
    scale_resize = int(256 / 224 * config.DATA.INPUT_SIZE)
    train_pipeline = [
        dict(type='DecordInit'),
        dict(type='SampleFrames', clip_len=config.DATA.CLIP_LEN, frame_interval=config.DATA.SAMPLE_RATE,
             num_clips=config.DATA.NUM_FRAMES, frame_uniform=config.DATA.FRAME_UNIFORM),
        dict(type='DecordDecode'),
        dict(type='RandAugment', auto_augment='rand-m7-n4-mstd0.5-inc1', input_size=None),
        dict(type='Resize', scale=(-1, scale_resize)),
        # dict(
        #     type='MultiScaleCrop',
        #     input_size=config.DATA.INPUT_SIZE,
        ...
        # dict(type='ColorJitter', p=config.AUG.COLOR_JITTER),
        # dict(type='GrayScale', p=config.AUG.GRAY_SCALE),
        # dict(type='RandAugment', auto_augment='rand-m7-n4-mstd0.5-inc1', input_size=config.DATA.INPUT_SIZE),
        dict(type='Normalize', **img_norm_cfg),
        dict(type='RandomErasing', probability=0.25),
...
def build_dataloader(logger, config):
    # NOTE: sampling strategy changes for T2D
    assert config.DATA.NUM_FRAMES == 1 or config.DATA.CLIP_LEN * config.DATA.SAMPLE_RATE == 1
    scale_resize = int(256 / 224 * config.DATA.INPUT_SIZE)

    if config.DATA.AUG_PIPELINES == 'k400':
        train_pipeline = [
            dict(type='DecordInit'),
            dict(type='SampleFrames', clip_len=config.DATA.CLIP_LEN, frame_interval=config.DATA.SAMPLE_RATE,
                 num_clips=config.DATA.NUM_FRAMES, frame_uniform=config.DATA.FRAME_UNIFORM),
    ...
    elif config.DATA.AUG_PIPELINES == 'ssv2_swin':
        train_pipeline = train_pipeline_ssv2_swin(config)
    elif config.DATA.AUG_PIPELINES == 'ssv2_slowfast':
        train_pipeline = train_pipeline_ssv2_slowfast(config)
    else:
        assert config.DATA.AUG_PIPELINES == 'ssv2'
        train_pipeline = train_pipeline_ssv2(config)
    ...
    val_pipeline = [
        dict(type='DecordInit'),
        dict(type='SampleFrames', clip_len=config.DATA.CLIP_LEN, frame_interval=config.DATA.SAMPLE_RATE,
             num_clips=config.DATA.NUM_FRAMES, test_mode=True, frame_uniform=config.DATA.FRAME_UNIFORM),
        dict(type='DecordDecode'),
        dict(type='Resize', scale=(-1, scale_resize)),
    ...
    if config.TEST.NUM_CLIP > 1:
        if config.DATA.NUM_FRAMES == 1:
            val_pipeline[1] = dict(type='SampleFrames', clip_len=config.DATA.CLIP_LEN,
                                   frame_interval=config.DATA.SAMPLE_RATE, num_clips=config.TEST.NUM_CLIP,
                                   test_mode=True, frame_uniform=config.DATA.FRAME_UNIFORM)
        else:
            val_pipeline[1] = dict(type='SampleFrames', clip_len=1, frame_interval=1, num_clips=config.DATA.NUM_FRAMES,
                                   ...
                                   frame_uniform=config.DATA.FRAME_UNIFORM)
