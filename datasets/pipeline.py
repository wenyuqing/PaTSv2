...
class SampleFrames:
    ...
    Args:
        clip_len (int): Frames of each sampled output clip_x.
    ...
    def _get_train_clips(self, num_frames):
        """Get clip_x offsets in train mode.
    ...
    def _get_test_clips(self, num_frames):
        """Get clip_x offsets in test mode.
    ...
    def _sample_clips(self, num_frames):
        """Choose clip_x offsets for the video in a given mode.
...
class RandAugment:
    def __init__(self, auto_augment, input_size=224, interpolation='bicubic', level='video'):
        if input_size is None:
            self.lazy_input_size = True
            input_size = 224
        else:
            self.lazy_input_size = False
    ...
    def __call__(self, results):
        if self.lazy_input_size:
            img_h, img_w, _ = results['imgs'][0].shape
            img_size_min = min(img_h, img_w)
            self.aa_params["translate_const"] = int(img_size_min * 0.45)
