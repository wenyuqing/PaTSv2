...
def _download(url: str, root: str = os.path.expanduser("~/.cache/clip_x")):
...
def load(model_path, name: str, device: Union[str, torch.device] = "cuda" if torch.cuda.is_available() else "cpu", 
    ...
    ----------
    name : str
        A model name listed by `clip_x.available_models()`, or the path to a model checkpoint containing the state_dict
