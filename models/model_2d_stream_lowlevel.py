...
class TransformerLow(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_type='full', enable_checkpoint=False):
        super().__init__()
        self.width = width
        self.layers = layers
...
class CLIP2DSLow(nn.Module):
    def __init__(self,
        ...
        self.transformer = TransformerLow(width, layers, heads, 'full', enable_checkpoint)
    ...
    def forward_features(self, x: torch.Tensor):
        ...
        for i in range(T):
            ...
            features = self.transformer(torch.cat([prompts, x[i]], dim=1), size)
            out=features[-1]
            prompts_out = self.prompts_projection(features[-4][:, :4])
