...
class Block_CA(nn.Module):
    ...
    def __init__(self, dim, num_heads=8,  qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 norm_layer=nn.LayerNorm, Attention_block=Class_Attention,init_values=1e-4
                 ):
        super().__init__()
    ...
    def forward(self, x_cls):
        ...
        x_cls = x_cls[:,-1:] + self.drop_path(self.gamma_1 * self.attn(self.norm1(x_cls)))
...
class CLIP2DSCLS12PCAV4(nn.Module):
    def __init__(self,
                 num_classes=400,
                 width=768,
                 patch_size=(2, 16, 16),
        ...
        if self.use_cls_token:
            print("!!!!!use cls token   12pppp cav3!!!!")
        ...
        self.prompts_ca=Block_CA(width, num_heads=heads)
    ...
    def forward_features(self, x: torch.Tensor):
        ...
        for i in range(T):
            ...
            if i>0:
                prompts_out_his[-1] = prompts_out_his[-1].detach()
            prompts_out_his.append(proj_out)
            ...
            for j,ln in enumerate(self.prompts_projection_ln):
                pout = ln(prompts_out[:,:,j])
