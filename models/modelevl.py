...
from .model_t2d import CLIPTextClassifier
...
class EVLTransformer(nn.Module):
    ...
    def __init__(
        ...
        use_text_classifier: bool = False,
        text_backbone_name: str = 'ViT-B/16',
        backbone_width=768,
        text_dim=512,
    ):
        super().__init__()
        ...
        if self.use_text_classifier:
            self.visual_proj = nn.Parameter(torch.randn(backbone_width, text_dim))
            self.head = CLIPTextClassifier(backbone_path, text_backbone_name, num_classes, bias=True)
        else:
